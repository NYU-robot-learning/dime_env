...
class BlockEnvV5(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, reward_type='sparse', random_start=0):
        ...
        mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/blockv5.xml', 5)
        ...
        self.action_space.high = np.ones_like(self.model.actuator_ctrlrange[:,1]) * 10
    ...
    def get_obs(self):
            ...
            return np.concatenate([qp, obj_vel, self.goal])
    ...
    def reset_model(self, seeds=None):
        self.offset1 = (np.random.random() - 0.5) * self.random_start 
        self.offset2 = (np.random.random() - 0.5) * self.random_start
        #back and forth
        self.init_qpos[17] = self.original_qpos[17] + self.offset1
    ...
    def set_env_state(self, state_dict):
        ...
        # qp[16:23] =  self.init_qpos[16:23].copy()
        qp =  self.init_qpos.copy()
        self.set_state(qp, qv)
    ...
    # ('robot0:FFJ3', 'robot0:FFJ2', 'robot0:FFJ1', 'robot0:FFJ0', 'robot0:MFJ3', 'robot0:MFJ2', 'robot0:MFJ1', 'robot0:MFJ0', 'robot0:LFJ3', 'robot0:LFJ2', 'robot0:LFJ1', 'robot0:LFJ0', 'robot0:THJ3', 'robot0:THJ2', 'robot0:THJ1', 'robot0:THJ0', 'OBJTx', 'OBJTy', 'OBJTz', 'OBJRx', 'OBJRy', 'OBJRz').
    ...
    def mj_render(self):
        ...
        super(BlockEnvV5, self).mj_render()

    def _set_target_to_goal(self):
        ...
        if self.target_position == 'ignore':
            ...
            goal[0] += 0.15
        # print('current: ' + str(self.sim.data.get_joint_qpos('target:joint')))
        # print('next: ' + str(goal))
    ...
    def _render_callback(self):
        self._set_target_to_goal()
        # print('after: ' + str(self.sim.data.get_joint_qpos('target:joint')))
        if 'object_hidden' in self.sim.model.geom_names:
            hidden_id = self.sim.model.geom_name2id('object_hidden')
            self.sim.model.geom_rgba[hidden_id, 3] = 1.
        self.sim.forward()

class DenseBlockEnvV5(BlockEnvV5):
    def __init__(self, reward_type='dense'):
        super(DenseBlockEnvV5,self).__init__(reward_type='dense')

class RandomStartEnvV5(BlockEnvV5):
    def __init__(self):
        super(RandomStartEnvV5,self).__init__(reward_type='dense')
        self.random_start = 0.1
        
    def set_random_start(self, random_start):
         super(RandomStartEnvV5,self).set_random_start(random_start)
