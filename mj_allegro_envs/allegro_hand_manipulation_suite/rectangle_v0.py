...
class RectangleEnvV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        self.tar_t_sid = 0
        self.tar_b_sid = 0
        self.rect_length = 1 #gets assigned further down
        self.tar_length = 1
        self.desired_orien = euler2quat(np.array([0, 3.14/2, 3.14]))

        curr_dir = os.path.dirname(os.path.abspath(__file__))
        mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/rectangle.xml', 5)

        # change actuator sensitivity
        self.sim.model.actuator_gainprm[self.sim.model.actuator_name2id('robot0:A_FFJ3'):self.sim.model.actuator_name2id('robot0:A_THJ0')+1,:3] = np.array([1, 0, 0])
        self.sim.model.actuator_biasprm[self.sim.model.actuator_name2id('robot0:A_FFJ3'):self.sim.model.actuator_name2id('robot0:A_THJ0')+1,:3] = np.array([0, -1, 0])
        ...
        self.rect_length = np.linalg.norm(self.data.site_xpos[self.obj_t_sid] - self.data.site_xpos[self.obj_b_sid])
        ...
        self.action_space.high[1] = 3.75
        self.action_space.high[5] = 3.75
        self.action_space.high[9] = 3.75
        self.action_space.high[13] = 3.75
    ...
    def step(self, a):
        ...
        obj_orien = (self.data.site_xpos[self.obj_t_sid] - self.data.site_xpos[self.obj_b_sid])/self.rect_length
        ...
        # penalty for dropping the rectangle
        done = False
        if obj_pos[2] < 0.15:
            reward -= 100

        goal_achieved = True if (dist < 0.175 and orien_similarity > 0.95) else False        
    ...
    def set_target_orien(self):
        ...
        self.model.body_quat[self.target_obj_bid] = self.desired_orien #euler2quat(self.desired_orien)
    ...
    def evaluate_success(self, paths):
        num_success = 0
        num_paths = len(paths)
        # success if rect within 15 degrees of target for 20 steps
        for path in paths:
